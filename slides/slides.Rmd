...
Interactive R: 
 ...
 - Open the Rgui.exe (on Windows) or 
 - Open Rstudio (recommended) - integrated developement environment for R
...
isTRUE(x) & is x TRUE 
\end{tabular}
\caption{Relational/Logical Operators}
\end{table}
...
Arithmetic/logical operators are \textbf{vectorized}: vector in -> vector out
...
Example: Store the above numeric vector by assinging it a name
...
Write a function that takes no arguments and adds 1 to 'a' if it exists, 
or sets a = 1 if it does not exist. exists() returns TRUE if its argument exists 
in memory and zero else.
...
```{r, echo=TRUE, eval=FALSE}
flights %>%                 # data
  filter(dest == "DTW" & month == 6)   # filter DTW as dest.
```
...
```{r, echo=TRUE, eval=FALSE}
fd6 = flights %>%                 # data
  filter(dest == "DTW") %>%  # filter DTW as dest.
...
```{r, echo=TRUE, eval=FALSE}
flights %>% filter(!is.na(dep_delay)) %>% select(dep_delay) %>%
...
Challenge in figuring out how to model real world processes with numbers
and variables

\bigskip

Note: randomness in R is pseudo-random. The default random number generator depends
on a seed which is some initial starting value where random numbers grow from.
Always set a seed.

## Simulation Basics

 - Often interested in expected values of random variables or probability of events 
 occuring
...
Instead:
 ...
 2. From sample, calculate the mean over the random draws to estimate expected value,
    or, calculate an observed frequency to estimate the probability.
...
```
 ...
 - Exercise: calculate the probability that the number of girls born will exceed
half the total population size.

```{r, echo=TRUE, eval=FALSE}
# estimate the probability by calculculating the desired 
# frequency 
...
sim = replicate(1000,
                {
                  isert simulation code here
                })
```

Example: use the sample() function to draw a collection of M/F values where the 
probability of F is 0.75. What is the probability of getting more males than females
in a set of 10 random people?

```{r, echo = TRUE, eval = FALSE}
experiment = replicate(100000, {
